...
def _do_refresh() -> str:
    """Core refresh logic without lock acquisition. Returns new session token.
    Used both standalone and as auto-refresh callback (which already holds lock)."""
    cfg = read_config()
...
def run(headless: bool = True, _hold_lock: bool = True) -> None:
    """Standalone refresh command. Set _hold_lock=False when called from an
    already-locked context (e.g. push's auto-refresh callback)."""
    if not _hold_lock:
        _do_refresh()
        return
    paths = config_paths()
